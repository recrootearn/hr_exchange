...
class DelhiveryService:
    ...
    def create_pickup_request(
        ...
        if not pickup_date:
            raise Exception(
                "Delhivery pickup date is required."
            )

        if not pickup_time:
            raise Exception(
                "Delhivery pickup time is required."
            )
        ...
        if not expected_package_count:
            raise Exception(
                "Delhivery expected package count is required."
            )
        ...
        try:
            ...
            raise Exception(
                "Invalid response from Delhivery pickup request: "
                + response.text
            )
...
def extract_delhivery_pickup_request_id(data):
...
def normalize_delhivery_status(raw_status):
    ...
    if "returned" in status and "rto" in status:
        return "Returned RTO"

    if "in transit" in status and "rto" in status:
    ...
    if "in transit" in status or status == "transit":
        return "In Transit"

    if "picked" in status or "picked up" in status:
        return "In Transit"

    if "ready for pickup" in status or "ready to pickup" in status:
        return "Ready for Pickup"

    if "ready to ship" in status or "manifest" in status:
...
def should_update_delhivery_status(
    current_status,
    new_status
):
    ...
    current_rank = DELHIVERY_STATUS_RANK.get(
        current_status,
        0
    )

    new_rank = DELHIVERY_STATUS_RANK.get(
        new_status,
        0
    )
...
def extract_delhivery_tracking_status(data):

    # Delhivery tracking responses commonly expose the current
    # shipment status inside ShipmentData -> Shipment -> Status.
    # The recursive fallback also handles account/response variations.
    ...
    return normalize_delhivery_status(value)


def sync_delhivery_order(order):
    ...
    try:
        ...
        tracking = delhivery.track_shipment(
            order.delhivery_waybill
        )

        order.tracking_json = json.dumps(
            tracking,
            default=str
        )

        order.last_tracking_sync = india_time()

        new_status = extract_delhivery_tracking_status(
            tracking
        )
        ...
        if not should_update_delhivery_status(
            old_status,
            new_status
        ):
            db.session.commit()
        ...
        if new_status == "In Transit":
            ...
            if not order.out_for_delivery_at:
        ...
        db.session.add(
            OrderStatusHistory(
                ...
                status=new_status,
                remarks=(
                    "Automatically updated from Delhivery tracking."
                )
            )
        )
        ...
        app.logger.exception(
            "Delhivery tracking sync failed for order %s: %s",
            order.id,
            e
        )
...
def candidate_feed():
    ...
    # SQLAlchemy expects a SQL expression there.
    # -------------------------------------------------
    ...
    # -------------------------------------------------
    # Sparked candidate videos
    # -------------------------------------------------
    sparked_candidate_video_ids = set()
    candidate_id = session.get("candidate_id")

    if candidate_id:
        sparked_candidate_video_ids = {
            s.video_id
            for s in CandidateVideoSpark.query.filter_by(
                candidate_id=candidate_id
            ).all()
        }
...
def create_delhivery_shipment(order_id):
    ...
    # -----------------------------------------------------
    # PREVENT DUPLICATE DELHIVERY SHIPMENTS
    # -----------------------------------------------------

    # If the shipment already exists but pickup scheduling failed,
    # this route is allowed to continue and retry only the pickup.
    # If both AWB and pickup request already exist, nothing is recreated.

    shipment_already_exists = bool(
        order.delhivery_waybill
    )

    if (
        shipment_already_exists
        and order.pickup_request_id
    ):

        flash(
            "Delhivery shipment and pickup request already exist for this order.",
            "warning"
        )
    ...
    if not pickup.pickup_location_code:
        ...
        pickup.pickup_location_code = (
            f"RECROOTEARN_SELLER_{order.seller_id}"
        )
    ...
    try:
        ...
        if shipment_already_exists:

            # Shipment was created previously. Do not create another
            # AWB; only continue with pickup scheduling below.
            data = None
            waybill = str(
                order.delhivery_waybill
            )
    ...
    if not shipment_already_exists:

        waybill = None

    if not shipment_already_exists and isinstance(data, dict):
    ...
    if not waybill:
        ...
        flash(
            "Delhivery response received, but waybill was not found. Please check the Delhivery response.",
            "warning"
        )
    ...
    order.shipping_provider = "delhivery"
    ...
    db.session.add(
        OrderStatusHistory(
            ...
            remarks=(
                "Delhivery shipment created and AWB generated."
            )
        )
    )
    ...
    # CREATE PICKUP REQUEST
    # -----------------------------------------------------
    # Existing UI uses a GET route, so no existing button/form
    # is disturbed. The default pickup is tomorrow at 11:00 AM.
    # A pickup date/time can also be supplied through query params:
    # ?pickup_date=YYYY-MM-DD&pickup_time=HH:MM:SS
    # -----------------------------------------------------

    try:

        requested_pickup_date = request.args.get(
            "pickup_date"
        )

        requested_pickup_time = request.args.get(
            "pickup_time"
        )

        if requested_pickup_date:

            pickup_date = requested_pickup_date
        ...
        # Validate supplied/default time.
        datetime.strptime(
            pickup_time,
            "%H:%M:%S"
        )

        # This application creates one Delhivery shipment per order,
        # so the pickup request represents one package/shipment.
        expected_package_count = 1

        pickup_response = (
            delhivery.create_pickup_request(
        ...
        pickup_request_id = (
            extract_delhivery_pickup_request_id(
                pickup_response
            )
        )

        # Keep the complete API response in the existing
        # tracking_json column, so no new database migration
        # is required for pickup date/time/response data.
        order.tracking_json = json.dumps(
            {
                "shipment_creation": data,
                "pickup_request": pickup_response,
                "pickup_date": pickup_date,
                "pickup_time": pickup_time,
                "pickup_location": (
                    pickup.pickup_location_code
        ...
        db.session.add(
            OrderStatusHistory(
                ...
                remarks=(
                    "Delhivery pickup scheduled for "
                    + pickup_date
                    + " at "
                    + pickup_time
                    + ". Pickup ID: "
        ...
        if pickup_request_id:
            ...
            flash(
                "Delhivery shipment and pickup scheduled successfully. AWB: "
                + str(waybill)
                + " | Pickup ID: "
                + str(pickup_request_id),
                "success"
            )
            ...
            flash(
                "Delhivery shipment and pickup were created, but Delhivery did not return a pickup request ID.",
                "warning"
            )

    except Exception as e:

        # The shipment/AWB is already valid. Do not delete it just
        # because pickup scheduling failed. Keep the order at
        # Ready to Ship so the pickup can be retried safely.
        ...
        try:
            ...
            db.session.add(
                OrderStatusHistory(
                    ...
                    status="Ready to Ship",
                    remarks=(
                        "Delhivery shipment created, but pickup scheduling failed: "
                        + str(e)
                    )
                )
            )
        ...
        flash(
            "Delhivery shipment was created, but pickup scheduling failed: "
            + str(e),
            "danger"
        )
...
# =========================================================
# DELHIVERY MANUAL TRACKING UPDATE
# =========================================================

@app.route(
    "/update-delhivery-tracking/<int:order_id>"
)
@login_required
def update_delhivery_tracking(order_id):
    ...
    if order.shipping_provider != "delhivery":

        flash(
            "This order is not using Delhivery.",
            "warning"
        )
    ...
    if not order.delhivery_waybill:
        ...
        flash(
            "Delhivery AWB is not available.",
            "warning"
        )
    ...
    if changed:
        ...
        flash(
            "Delhivery tracking updated successfully.",
            "success"
        )
        ...
        flash(
            "Delhivery tracking checked. No new status was available.",
            "info"
        )
...
@app.route(
    "/internal/delhivery-sync",
    methods=["POST"]
)
def internal_delhivery_sync():

    sync_secret = os.environ.get(
        "DELHIVERY_SYNC_SECRET",
        ""
    )
    ...
    if not sync_secret:
        ...
        return jsonify({
            "success": False,
            "error": "DELHIVERY_SYNC_SECRET is not configured."
        }), 500
    ...
    orders = Order.query.filter(
        Order.shipping_provider == "delhivery",
        Order.delhivery_waybill.isnot(None),
